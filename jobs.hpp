...
namespace NP {
	...
	*/
	...
	private:
		...
		JobID id;
		JobType jobType;
		//std::set<Job*> incompatible; //litle omega
		hash_value_t key;
	...
	public:
		...
		Job(unsigned long id,
			Interval<Time> arr, Interval<Time> cost, Time dl, Priority prio, JobType type, unsigned long tid = 0)
			: arrival(arr), cost(cost), deadline(dl), priority(prio), jobType(type), id(id, tid)
		{
			compute_hash();
		}

/*		void addIncomatibility(Job<Time> *incompatibility)
		{
			incompatible.insert(incompatibility);
		}

		std::set<Job*> getOmega()
		{
			return incompatible;
		}
		*/
