...
namespace NP {
	...
	/*
	* This class defines a Job for conditional DAG tasks
	*
	*
	*
	*/

	template<class Time> class JobCond {

	public:
		typedef std::vector<JobCond<Time>> Job_set;
	...
	private:
		...
		JobID id;
		JobType jobType;
		std::set<JobCond*> incompatible; //litle omega



	public:

		JobCond(unsigned long id,
			Interval<Time> arr, Interval<Time> cost, Time dl, Priority prio, JobType type, std::set<JobCond*> incompatible, unsigned long tid = 0)
			: arrival(arr), cost(cost), deadline(dl), priority(prio), jobType(type), incompatible(incompatible), id(id, tid)
		{
		...
		bool higher_priority_than(const JobCond& other) const
		{
		...
		bool priority_at_least_that_of(const JobCond& other) const
		{
			return priority <= other.priority;
		}
		...
		static Interval<Time> scheduling_window(const JobCond& j)
		{
			return j.scheduling_window();
		}

		friend std::ostream& operator<< (std::ostream& stream, const JobCond& j)
		{
	...
	template<class Time>
	bool contains_job_with_id(const typename JobCond<Time>::Job_set& jobs,
		const JobID& id)
	{
		auto pos = std::find_if(jobs.begin(), jobs.end(),
			[id](const JobCond<Time>& j) { return j.is(id); });
		return pos != jobs.end();
	}
	...
	template<class Time>
	const JobCond<Time>& lookup(const typename JobCond<Time>::Job_set& jobs,
		const JobID& id)
	{
		auto pos = std::find_if(jobs.begin(), jobs.end(),
			[id](const JobCond<Time>& j) { return j.is(id); });
