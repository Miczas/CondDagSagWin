...
namespace NP {
	...
	namespace Global {
		...
		typedef std::set<Job_index> Job_precedence_set;
		typedef std::set<Job_index> Job_incompatibility_set;
		...
		template<class Time> class Schedule_state
		{
			...
			, lookup_key{(NP::hash_value_t)0x9a9a9a9a9a9a9a9aUL}
			{
				sizeOfOmega = 0;
			...
			, lookup_key{from.lookup_key ^ key}
			{
				...
				computeOmega(incompatible);
				sizeOfOmega = Omega.size();

				DM("*** new state: constructed " << *this << std::endl);
			}

			//necessary to use (ordered) multisets of states
			int sizeOfOmega;
			bool operator<(const Schedule_state& st) const { return sizeOfOmega < st.sizeOfOmega; }
			...
			bool same_jobs_incompatible(const Schedule_state& other) const
			{
				return this->get_Omega() == other.get_Omega();
			}

			const Index_set* jobs_scheduled() const
			{
			...
			bool can_merge_with(const Schedule_state<Time>& other) const
			{
				...
				//	return false;
				if (!same_jobs_incompatible(other))
					return false;
			...
			bool check(const Schedule_state<Time>& other) const
			{
			...
			Job_incompatibility_set get_Omega() const
			{
				return Omega;
			}
			...
			void computeOmega(const std::vector<Job_incompatibility_set>& incompatible) 
			{
				const Index_set* jobsScheduled = jobs_scheduled();
